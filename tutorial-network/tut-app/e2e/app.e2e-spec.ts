...
describe('Starting tests for tut-app', function() {
  ...
  it('website title should be tut-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tut-app');
    })
  });
  ...
  it('navbar-brand should be tut-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tut-app');
    });
  });
